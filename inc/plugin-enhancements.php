...
class Theme_Plugin_Enhancements {
	...
	function __construct() {
		...
		if ( empty( $this->dependencies ) ) :
			...
			$this->plugins = array(
	    		...
	    		'message' => sprintf(
					__( 'The %1$s is required to use some of this theme&rsquo;s features, including: ', 'generate_prf' ),
				'<strong>' . __( 'Jetpack plugin', 'generate_prf' ) . '</strong>' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			   'none'                 => __( 'no specific module needed', 'generate_prf' ),
			   'custom-content-types' => __( 'Custom Content Types module', 'generate_prf' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please activate %1$s. %2$s', 'generate_prf' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . __( 'Activate', 'generate_prf' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please install %1$s. %2$s', 'generate_prf' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . __( 'Install', 'generate_prf' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'generate_prf' ),
